...
function createContactBlock<T extends HTMLElement>(tagName: string): T {
  return document.createElement(tagName) as T;
}

const allContacts = (contacts: ContactLists[]) => {
  if (contactsContainer) {
    contactsContainer.innerHTML = "";
  }

  let lists = contacts.map(({ name, phone, picture }: ContactLists) => {
    const list = createContactBlock<HTMLDivElement>("div");
    const contactDetails = createContactBlock<HTMLDivElement>("div");
    const contactName = createContactBlock<HTMLParagraphElement>("p");
    const contactNumber = createContactBlock<HTMLParagraphElement>("p");
    const contactPic = createContactBlock<HTMLImageElement>("img");
...
const handle_duplicates = (name: string, number: string) => {
  for (const contact of contacts) {
    if (name.toLowerCase() === contact.name.toLowerCase()) {
      if (
        window.confirm(
          "Contact already Exists, Do you want to update the phone number",
        )
      ) {
        if (contactsContainer) {
          contactsContainer.innerHTML = "";
...
const add_new_contact = () => {
  if (name_input && number_input) {
    if (name_input.value.trim() !== "" && number_input.value.trim() !== "") {
      ...
      number_input.value = "";
      contactPanel?.classList.remove("open");
    } else alert("Both the name and the number must be valid");
  }
};
...
const queryList = (query: string | undefined) => {
  const normalizedQuery = query?.toLowerCase() ?? "";
  const qResult = contacts.filter((contact) =>
